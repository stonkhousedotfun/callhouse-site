...
export default async function HomePage() {
  ...
  const headline = "TradFi on RobinHood";

  return <>
    <Container as="section" aria-labelledby="hero-h"
      className={`grid grid-cols-1 items-center gap-9 pb-[72px] pt-4 lg:gap-14 lg:pt-10${hero ? " lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]" : ""}`}>
      <div>
        <Chip tone="accent" dot wrap>{`${LAUNCH_SET.join(" · ")} · ${CHAIN_NAME}`}</Chip>
        ...
        <p className="mt-6 max-w-[35em] text-[19px] text-ink-2">
          Small bets on big stocks. Earn interest on your stocks, or own a slice of the casino.
        </p>
      ...
      {hero ? <PayoffCard card={hero} featured /> : null}
    </Container>
    ...
    <Section id="demo" labelledBy="demo-h">
      <SectionHead id="demo-h" eyebrow="Try a scenario" title="Move the price. Watch the payoff."
        intro="Same fee-net maths as the app." />
      <PayoffDemo />
    </Section>
