...
const WHAT_WE_DO: readonly { title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: `Each week it offers calls ${fmtPct(5)} to 11.5% above spot, up to ${fmtPct(95)} of the stock, and writes a call only in the same transaction a buyer pays for it. A week with no buyer writes nothing.`,
  },
  {
...
const STEPS: readonly { title: string; when: string; body: string; key?: boolean }[] = [
  ...
  },
  {
    ...
    body: `A call ${fmtPct(5)} to 11.5% above spot, inside a ${fmtPct(3)} to ${fmtPct(12)} band. Nothing is written yet.`,
    key: true,
  },
  {
...
const BENEFITS: readonly { title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: "The app's activity page lists each week the vault arms: filled, unfilled or assigned, zeros included. A week the keeper skips leaves no row. No projections.",
  },
];
...
const RISKS: readonly { title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: `No external audit, only an internal review dated 14 September 2026, and no bug bounty. An external audit is pending. There is no proxy, so a fix means a new vault. Deposits are capped at 20 ${MARKET}.`,
  },
];
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          <article className="rounded-lg border border-accent/30 bg-accent-soft p-[26px]">
            ...
            <p className="mt-2 max-w-[34em] text-[15.5px] text-ink-2">
              Nvidia Stock Tokens, shares of {SHARE_TICKER}. Cap 20 {MARKET}, protocol fee {fmtPct(5)} of premium,
              strike {fmtPct(5)} to 11.5% above spot inside a {fmtPct(3)}–{fmtPct(12)} band. Same week, every week, New
              York time.
            </p>
          ...
          <article className="rounded-lg border border-dashed border-line-2 bg-surface p-[26px]">
            ...
            <p className="mt-2 max-w-[34em] text-[15.5px] text-ink-2">
              Additional Stock Token vaults on {CHAIN_NAME}, one underlying each, after the audit report is public. We
              will not name the next ticker until that vault is actually being built.
            </p>
